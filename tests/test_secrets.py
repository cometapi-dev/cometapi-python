...
def test_scope_scan_rejects_oidc_on_an_unreviewed_workflow(tmp_path: Path) -> None:
    ...
    for name in ("ci.yml", "publish.yml", "release-please.yml"):
...
def test_scope_scan_rejects_missing_reusable_caller_oidc(tmp_path: Path) -> None:
    workflows = tmp_path / ".github" / "workflows"
    workflows.mkdir(parents=True)
    for name in ("ci.yml", "publish.yml", "release-please.yml"):
    ...
    assert scan_workflow_scope(tmp_path) == [
        ".github/workflows/release-please.yml: id-token: write must match the reviewed "
